...
def get_naming_series_for_sales_order() -> str:
    """
    Unlike Quotation (which branches naming series based on document_type
    i.e. Quotation vs Proforma Invoice), Sales Order has a single document
    type, so we simply return the default (first) naming series option.
    """
    meta = frappe.get_meta("Sales Order")
    naming_series_field = meta.get_field("naming_series")

    if not naming_series_field or not naming_series_field.options:
        frappe.throw("Naming Series options for Sales Order are not configured in the system.")

    series_list = [s.strip() for s in naming_series_field.options.split('\n') if s.strip()]

    if not series_list:
        frappe.throw("Naming Series options for Sales Order are empty. Please configure them in Document Naming Settings.")

    return series_list[0]


def validate_sales_order_payload(data: Dict[str, Any], is_update=False):
    ...
    if posting_date and delivery_date:
        if delivery_date < posting_date:
            raise frappe.ValidationError("Delivery Date cannot be before Posting Date.")
...
def sync_sales_order_terms(sales_order, terms_payload):
    ...
    if phases:
        ...
        if round(total_pct, 2) == 100.00:
            ...
            sales_order.payment_terms_template = pt_doc.name
            sales_order.set("payment_schedule", [])

            base_date = sales_order.transaction_date or frappe.utils.today()
    ...
    tc_name = f"{sales_order.name} Terms"
    ...
    sales_order.tc_name = tc_name
    sales_order.terms = tc_content
    is_so_dirty = True
...
def sync_taxes(sales_order, data):
    sales_order.set("taxes", [])

    default_cc = sales_order.get("cost_center") or frappe.get_cached_value(
        "Company", sales_order.company, "cost_center"
    )
    existing_heads = set()
    is_dirty = False

    template_name = data.get("salesTaxTemplate") or sales_order.taxes_and_charges
    ...
    for item in (getattr(sales_order, "items", None) or []):
    ...
    if tax_overrides:
        ...
        for tax_row in (getattr(sales_order, "taxes", None) or []):
...
def build_sales_order_filters(args):
    frappe_filters = {}
...
def _build_sales_order_box_detail(item: dict) -> dict:
    return {
