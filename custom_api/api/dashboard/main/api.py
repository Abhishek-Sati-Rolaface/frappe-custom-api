        ...
        top_customers_list = [
            {
                ...
                "value": flt(row.total_sales)
            } for row in top_customers_query
        ]

        top_suppliers_list = [
            {
                "name": row.supplier_name or "N/A", 
                "value": flt(row.total_purchases)
            } for row in top_suppliers_query
        ]

        notes_data = {
            "topCustomers": top_customers_list, # Now returns an array of up to 3 objects
            "topSuppliers": top_suppliers_list, # Now returns an array of up to 3 objects
            "topSellingItemQty": {
...
def sales_chart(from_date=None, to_date=None, year=None, interval="Monthly"):
    try:
        ...
        if interval == "Monthly":
            for m in months:
                trend_data[m] = {"receivable": 0.0, "received": 0.0}
        elif interval == "Quarterly":
            for q in ["Q1", "Q2", "Q3", "Q4"]:
                trend_data[q] = {"receivable": 0.0, "received": 0.0}
        elif interval == "Half-Yearly":
            for h in ["H1", "H2"]:
                trend_data[h] = {"receivable": 0.0, "received": 0.0}
